...
static int dprintf(int fd, const char *fmt, ...) {
  ...
  va_end(ap);
  if (r >= 0 && r < sizeof buf) return write(fd, buf, r);
  return -1;
}
#endif

static void swrite(int fd, char *str) {
  size_t l = strlen(str);
...
static void write_execline(int fd, int argc, char *argv[]) {
  int i;
  char *s;
...
static void setx(int fd, int executable) {
  struct stat st;
  fstat(fd, &st);
  if (executable)
    st.st_mode |= 0100;
...
int main(int argc, char *argv[]) {
    ...
    if (WIFEXITED(status)) {
      ...
      if (WEXITSTATUS(status) == 0) {
        if (cflag)
          ...
          renameat(dirfd, lockfile, donedirfd, lockfile);
      }
      if (WEXITSTATUS(status) != 0) {
        if (failpath) renameat(dirfd, lockfile, faildirfd, lockfile);
  ...
  snprintf(lockfile, sizeof lockfile, ".,%011" PRIx64 ".%d", ms, getpid());
  lockfd = openat(dirfd, lockfile, O_CREAT | O_EXCL | O_RDWR | O_APPEND, 0666);
  if (lockfd < 0) {
    perror("open");
    exit(222);
  }
